import { useState } from "react";
import { useNavigate, useParams } from "react-router";
import { motion } from "motion/react";
import {
    ArrowLeft,
    Building2,
    GripVertical,
    FileArchive,
    Trash2,
    Eye,
    X,
    ClipboardList
} from "lucide-react";
import { InlineChatbot } from "../components/InlineChatbot";

interface SequencedDocument {
    id: string;
    order: number;
    name: string;
    size: string;
}

export function FinalBidSequence() {
    const navigate = useNavigate();
    const { tenderId } = useParams();

    // Mock Tender Data
    const tender = {
        id: tenderId || "1",
        title: "Industrial Valves Supply - Karnataka PWD",
        organization: "Karnataka Public Works Department",
    };

    const [viewingDoc, setViewingDoc] = useState<SequencedDocument | null>(null);

    const [documents, setDocuments] = useState<SequencedDocument[]>([
        { id: "1", order: 1, name: "Company Registration Certificate", size: "500kb" },
        { id: "2", order: 2, name: "GST Registration Certificate", size: "320kb" },
        { id: "3", order: 3, name: "Generated Application Bid Form", size: "850kb" },
    ]);

    // Simple swap function for demonstration (in a real app you'd use a DnD library like dnd-kit)
    const moveDocUp = (index: number) => {
        if (index === 0) return;
        const newDocs = [...documents];
        const temp = newDocs[index - 1];
        newDocs[index - 1] = newDocs[index];
        newDocs[index] = temp;
        // Update order numbers to visually reflect their new index
        setDocuments(newDocs.map((doc, i) => ({ ...doc, order: i + 1 })));
    };

    const moveDocDown = (index: number) => {
        if (index === documents.length - 1) return;
        const newDocs = [...documents];
        const temp = newDocs[index + 1];
        newDocs[index + 1] = newDocs[index];
        newDocs[index] = temp;
        // Update order numbers to visually reflect their new index
        setDocuments(newDocs.map((doc, i) => ({ ...doc, order: i + 1 })));
    };

    return (
        <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-blue-50">
            {/* Header */}
            <div className="bg-white border-b border-gray-200 px-4 sm:px-6 py-3 sm:py-4">
                <div className="max-w-7xl mx-auto flex items-center justify-between">
                    <div className="flex items-center gap-3 sm:gap-4">
                        <button
                            onClick={() => navigate(`/tender/${tenderId}/generated-documents`)}
                            className="p-2 hover:bg-gray-100 rounded-lg transition-colors"
                        >
                            <ArrowLeft className="w-5 h-5 text-gray-500" />
                        </button>
                        <div>
                            <h1 className="text-base sm:text-lg font-semibold text-indigo-600">
                                Tender Details
                            </h1>
                            <p className="text-[10px] sm:text-xs text-gray-400 font-medium tracking-wide uppercase">
                                REFERENCE ID: TND-{tender.id}
                            </p>
                        </div>
                    </div>
                </div>
            </div>

            {/* Main Content */}
            <div className="max-w-7xl mx-auto p-4 sm:p-6 mt-2 sm:mt-4">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-8">

                    {/* Left Column - Document Sequencer */}
                    <div className="lg:col-span-2 space-y-6">

                        {/* Header Block */}
                        <motion.div
                            initial={{ opacity: 0, y: 20 }}
                            animate={{ opacity: 1, y: 0 }}
                            transition={{ duration: 0.5 }}
                            className="bg-white rounded-xl sm:rounded-2xl shadow-lg border border-gray-100 p-4 sm:p-6 flex items-start sm:items-center gap-3 sm:gap-5"
                        >
                            <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-r from-indigo-600 to-blue-600 rounded-xl sm:rounded-2xl flex items-center justify-center flex-shrink-0 shadow-sm">
                                <Building2 className="w-6 h-6 sm:w-8 sm:h-8 text-white" />
                            </div>
                            <div className="min-w-0 flex-1">
                                <h2 className="text-base sm:text-[22px] font-bold text-gray-900 mb-1 line-clamp-2">{tender.title}</h2>
                                <div className="flex items-center gap-2">
                                    <span className="p-1 bg-gray-100 rounded text-gray-500 flex-shrink-0">
                                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /><line x1="16" y1="13" x2="8" y2="13" /><line x1="16" y1="17" x2="8" y2="17" /><polyline points="10 9 9 9 8 9" /></svg>
                                    </span>
                                    <p className="text-xs sm:text-sm font-medium text-gray-500 truncate">{tender.organization}</p>
                                </div>
                            </div>
                        </motion.div>

                        {/* Sequencer Container */}
                        <motion.div
                            initial={{ opacity: 0, y: 20 }}
                            animate={{ opacity: 1, y: 0 }}
                            transition={{ duration: 0.5, delay: 0.1 }}
                            className="bg-white rounded-xl sm:rounded-2xl shadow-lg border border-gray-100 p-4 sm:p-6 flex flex-col min-h-[400px] sm:min-h-[500px]"
                        >
                            {/* List Header */}
                            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-2 sm:gap-0 border-b border-transparent mb-4 sm:mb-6">
                                <h3 className="text-base sm:text-lg font-bold text-gray-900">
                                    Generated Bid Documents (Count: {documents.length})
                                </h3>
                                <p className="text-[11px] sm:text-[12px] italic text-gray-500 font-medium sm:pb-1">
                                    Arrange the Sequence of Documents (if needed)
                                </p>
                            </div>

                            {/* Reorderable List */}
                            <div className="flex-1 space-y-3 sm:space-y-4">
                                {documents.map((doc, index) => (
                                    <div key={doc.id} className="flex items-center gap-2 sm:gap-3">

                                        {/* Drag Handle (Visual only for now, mapped to up/down clicks) */}
                                        <div className="flex flex-col text-gray-300">
                                            <button onClick={() => moveDocUp(index)} disabled={index === 0} className="hover:text-blue-500 focus:outline-none disabled:opacity-30 disabled:hover:text-gray-300 -mb-1">
                                                <svg width="16" height="16" className="sm:w-[18px] sm:h-[18px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="18 15 12 9 6 15"></polyline></svg>
                                            </button>
                                            <button onClick={() => moveDocDown(index)} disabled={index === documents.length - 1} className="hover:text-blue-500 focus:outline-none disabled:opacity-30 disabled:hover:text-gray-300 -mt-1">
                                                <svg width="16" height="16" className="sm:w-[18px] sm:h-[18px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="6 9 12 15 18 9"></polyline></svg>
                                            </button>
                                        </div>

                                        {/* Number Badge */}
                                        <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-emerald-500 text-white font-bold flex flex-shrink-0 items-center justify-center text-sm sm:text-[18px] shadow-sm">
                                            {doc.order}
                                        </div>

                                        {/* Document Card */}
                                        <div className="flex-1 border border-blue-400 rounded-xl sm:rounded-full px-3 sm:px-5 py-2.5 sm:py-3 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 sm:gap-0 group hover:bg-blue-50/50 transition-colors min-w-0">
                                            <div className="font-semibold text-gray-800 text-xs sm:text-sm truncate">{doc.name}</div>

                                            <div className="flex items-center gap-2 sm:gap-5 flex-wrap sm:flex-nowrap">
                                                <div className="text-[11px] sm:text-[13px] font-bold text-gray-900 hidden xs:block">
                                                    File Size: <span className="font-medium text-gray-500">{doc.size}</span>
                                                </div>

                                                <div className="flex items-center gap-2 sm:gap-3 sm:border-l sm:border-gray-200 sm:pl-4">
                                                    <button
                                                        onClick={() => setViewingDoc(doc)}
                                                        className="flex items-center gap-1 sm:gap-1.5 text-[10px] sm:text-xs font-bold text-gray-700 hover:text-blue-600 transition-colors tooltip-trigger"
                                                        title="View Document"
                                                    >
                                                        <Eye className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> <span className="hidden xs:inline">View</span>
                                                    </button>
                                                    <button className="flex items-center gap-1 sm:gap-1.5 text-[10px] sm:text-xs font-bold text-gray-700 hover:text-blue-600 transition-colors tooltip-trigger" title="Compress File">
                                                        <FileArchive className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-600" /> <span className="hidden xs:inline">Compress</span>
                                                    </button>
                                                    <button className="text-red-500 hover:text-red-600 hover:bg-red-50 p-1 rounded transition-colors tooltip-trigger" title="Remove Document">
                                                        <Trash2 className="w-4 h-4 sm:w-5 sm:h-5" />
                                                    </button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                            </div>

                            {/* Footer Submit Button */}
                            <div className="flex justify-center pt-6 sm:pt-8 mt-4 border-t border-gray-100">
                                <button
                                    onClick={() => navigate(`/tender/${tenderId}/final-download-submission`)}
                                    className="w-full sm:w-auto px-6 sm:px-10 py-3 sm:py-3.5 rounded-xl bg-gradient-to-r from-indigo-600 to-blue-600 text-white font-bold text-sm sm:text-[16px] hover:from-indigo-700 hover:to-blue-700 transition-colors shadow-sm"
                                >
                                    Prepare Final Bid Document
                                </button>
                            </div>

                        </motion.div>
                    </div>

                    {/* Right Column - Actions Sidebar */}
                    <div className="lg:col-span-1 space-y-4">

                        {/* 1. Check Saved Bids Button */}
                        <motion.button
                            initial={{ opacity: 0, y: 20 }}
                            animate={{ opacity: 1, y: 0 }}
                            transition={{ duration: 0.5, delay: 0.1 }}
                            onClick={() => navigate("/saved-bids")}
                            className="w-full bg-white hover:bg-indigo-50 border border-indigo-200 text-indigo-700 font-bold px-4 sm:px-6 py-3 sm:py-4 rounded-xl sm:rounded-2xl flex items-center justify-center gap-2 sm:gap-3 shadow-sm transition-all hover:shadow-md text-sm sm:text-[16px]"
                        >
                            <ClipboardList className="w-4 h-4 sm:w-5 sm:h-5 text-indigo-700" strokeWidth={2.5} />
                            Check all saved bids
                        </motion.button>

                        {/* 2. Opportunity X AI Chatbot */}
                        <motion.div
                            initial={{ opacity: 0, y: 20 }}
                            animate={{ opacity: 1, y: 0 }}
                            transition={{ duration: 0.5, delay: 0.2 }}
                        >
                            <InlineChatbot />
                        </motion.div>

                    </div>
                </div>
            </div>

            {/* View Document Modal Overlay */}
            {viewingDoc && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-2 sm:p-4 lg:p-8">
                    <motion.div
                        initial={{ opacity: 0, scale: 0.95, y: 20 }}
                        animate={{ opacity: 1, scale: 1, y: 0 }}
                        exit={{ opacity: 0, scale: 0.95, y: 20 }}
                        className="bg-gray-100 rounded-xl sm:rounded-2xl shadow-2xl w-full max-w-5xl h-[95vh] sm:h-[90vh] flex flex-col overflow-hidden relative"
                    >
                        {/* Modal Header */}
                        <div className="bg-white border-b border-gray-200 px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between shrink-0">
                            <div className="min-w-0 flex-1 pr-3">
                                <h3 className="text-base sm:text-xl font-bold text-gray-900 line-clamp-1">{viewingDoc.name}</h3>
                                <p className="text-xs sm:text-sm text-gray-500">Document #{viewingDoc.order} • {viewingDoc.size}</p>
                            </div>
                            <div className="flex items-center gap-3">
                                <button
                                    onClick={() => setViewingDoc(null)}
                                    className="w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-full bg-gray-100 hover:bg-gray-200 text-gray-600 transition-colors"
                                    title="Close Preview"
                                >
                                    <X className="w-4 h-4 sm:w-5 sm:h-5" />
                                </button>
                            </div>
                        </div>

                        {/* Document Content Mock Canvas (A4 Aspect Ratio) */}
                        <div className="p-3 sm:p-8 flex-1 overflow-y-auto custom-scrollbar">
                            <div className="bg-white max-w-[800px] min-h-[600px] sm:min-h-[1050px] mx-auto shadow-sm border border-gray-300 p-4 sm:p-12 text-gray-800 relative">

                                {/* Official Tender Header */}
                                <div className="border-b-2 border-gray-800 pb-4 sm:pb-6 mb-4 sm:mb-8 text-center flex flex-col items-center">
                                    <img
                                        src="https://upload.wikimedia.org/wikipedia/commons/thumb/5/55/Emblem_of_India.svg/200px-Emblem_of_India.svg.png"
                                        alt="Emblem"
                                        className="h-10 sm:h-16 mb-2 sm:mb-4 opacity-80 mix-blend-multiply"
                                    />
                                    <h2 className="text-base sm:text-2xl font-bold uppercase tracking-wider sm:tracking-widest text-gray-900">Government of Karnataka</h2>
                                    <h3 className="text-sm sm:text-lg text-gray-700 font-semibold mb-1">Public Works Department (PWD)</h3>
                                    <p className="text-xs sm:text-sm text-gray-500 font-medium">Official Bid Document Submission Form</p>
                                </div>

                                <div className="space-y-4 sm:space-y-6 text-xs sm:text-[15px] leading-relaxed relative z-10">

                                    {/* Tender Meta Info Box */}
                                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 sm:gap-0 mb-4 sm:mb-8 bg-gray-50 border border-gray-200 p-3 sm:p-4 rounded-lg">
                                        <div>
                                            <p className="font-bold text-gray-900 text-sm sm:text-lg mb-1">
                                                Tender Ref: TND-{tender.id}
                                            </p>
                                            <p className="text-gray-700 font-semibold text-xs sm:text-sm">Project: {tender.title}</p>
                                        </div>
                                        <div className="sm:text-right text-xs sm:text-sm">
                                            <p><strong>Date:</strong> {new Date().toLocaleDateString()}</p>
                                            <p><strong>Document ID:</strong> DOC-{viewingDoc.id}-2025</p>
                                        </div>
                                    </div>

                                    <h1 className="text-sm sm:text-xl font-bold uppercase tracking-wider mb-4 sm:mb-6 text-center border-y border-gray-300 py-2 sm:py-3 bg-gray-50">
                                        {viewingDoc.name}
                                    </h1>

                                    <p>
                                        To,<br />
                                        The Chief Engineer,<br />
                                        Karnataka Public Works Department
                                    </p>

                                    <p className="mt-4 sm:mt-6 text-justify">
                                        Dear Sir/Madam,
                                        <br /><br />
                                        We hereby submit our finalized documentation for the <strong>"{tender.title}"</strong> project as requested in the official tender notice.
                                        This document, classified as <em>"{viewingDoc.name}"</em>, conforms to all standard terms and conditions mandated by the statutory regulations.
                                        <br /><br />
                                        Please find the details associated with this stage of the bidding sequence. All attached declarations, stamps, and digital sign-offs are valid through the duration of the review period.
                                    </p>

                                    {/* Mock text generation to fill out the page */}
                                    <div className="space-y-4 text-gray-600 mt-4 sm:mt-8">
                                        <p className="text-justify bg-gray-50 p-3 sm:p-4 rounded-lg border border-gray-100 text-xs sm:text-sm">
                                            1. The bidder agrees to abide by all the terms and conditions outlined in the aforementioned tender document.
                                            <br /><br />
                                            2. It is certified that all the information provided herein is true and correct to the best of our knowledge. Any discrepancies found may lead to the immediate cancellation of the bid.
                                            <br /><br />
                                            3. The quoted rates shall remain valid for a period of 120 days from the date of opening of the financial bid.
                                            <br /><br />
                                            4. All statutory clearances, safety compliance, and labor laws shall be strictly adhered to during the execution of this contract.
                                        </p>
                                    </div>
                                </div>

                                <div className="mt-10 sm:mt-20 pt-6 sm:pt-10 border-t border-gray-400 grid grid-cols-2 gap-4 sm:gap-8 text-xs sm:text-sm">
                                    <div className="text-center">
                                        <div className="h-10 sm:h-16 w-20 sm:w-32 mx-auto border-b border-gray-400 mb-2"></div>
                                        <p className="font-bold text-gray-800">Authorized Signatory</p>
                                        <p className="text-gray-500 text-[10px] sm:text-sm">Applicant Company</p>
                                    </div>
                                    <div className="text-center">
                                        <div className="h-10 sm:h-16 w-20 sm:w-32 mx-auto border-b border-gray-400 mb-2 items-center flex justify-center text-2xl sm:text-4xl text-blue-100 opacity-50"><Building2 /></div>
                                        <p className="font-bold text-gray-800">Company Stamp/Seal</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </motion.div>
                </div>
            )}

            <style>{`
        .custom-scrollbar::-webkit-scrollbar {
          width: 14px;
        }
        .custom-scrollbar::-webkit-scrollbar-track {
          background: #EEF2FF;
          border-radius: 10px;
        }
        .custom-scrollbar::-webkit-scrollbar-thumb {
          background: #9CA3AF;
          border-radius: 10px;
          border: 3px solid #EEF2FF;
        }
      `}</style>
        </div>
    );
}
